...
def main():
    """"""
    qapp = create_qapp()                                    # 创建pyqt界面

    event_engine = EventEngine()                            # 事件引擎

    main_engine = MainEngine(event_engine)                  # 主引擎

    main_engine.add_gateway(CtpGateway)                     # 增加网关引擎（CTP）
    # main_engine.add_gateway(CtptestGateway)               # 这个应该是那个测试连接模块
    ...
    main_engine.add_app(CtaStrategyApp)                     # APP模块
    ...
    main_window = MainWindow(main_engine, event_engine)     # 启动主窗口，并且配置事件引擎和主引擎
